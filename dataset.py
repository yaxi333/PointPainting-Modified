...
class KittiDataset(torch.utils.data.Dataset):
    ...
    def augment_lidar_class_scores(self, class_scores, lidar_cam_coords, projection_mats):
        ...
        ### CHANGED
        points_projected_on_mask = points_projected_on_mask.type(torch.int64)
        point_scores = class_scores[points_projected_on_mask[:, 1], points_projected_on_mask[:, 0]].type(torch.int64)
        ### CHANGED
        point_scores = point_scores.reshape(-1, 1).double()
